\begin{tikzpicture}[auto, rounded corners=0, x = 1cm, y = 1cm] {%
    ...
    \node [action, below = of src] (reading-in) {Read-in graph and add placeholder for new metric};
    ...
    \coordinate [left = of graph] (dst);

    \node [data, below = of contracting] (ch-graph) {$G_{CH}=(V,E_{CH})$};
    \node [action, below = of ch-graph] (explorating) {Explore given \glspl{stpair} with \gls{personalized_routing} and choose one path u.a.r.};
