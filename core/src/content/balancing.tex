...
Further details about the testing-machine and experimental results can be found in chapter~\ref{chap:experiments}.
...
This will also be pointed out in the respective experiments in chapter~\ref{chap:experiments}.
...
\section{Balancing to create a new metric}
    ...
    This \gls{balancing} is depicted in figure~\ref{fig:balancing} and explained briefly in the caption or detailled in the following.
    ...
    \subsection{Selection of route-pairs}
        ...
        \todo{TODO @Florian: Opinion about performance-issues below and how they are described?}
    ...
    \subsection{First steps of the balancing-process}
        ...
        Non-normalized \glspl{metric} of different scales would cause \gls{repr} to find \gls{alpha}-values of different scale, making some \glspl{metric} more important than others just based on their different scales.
        ...
        \todo{TODO @Florian Zum folgenden Absatz: Kommt sowas hier schon rein oder ist das zu nah an den Experimenten und sollte entsprechend in das Kapitel für Experimente?}
        ...
        Both methods are compared in the experiments in chapter~\ref{chap:experiments}.
    ...
    \subsection{Updating the new metric with the collected workload}
        ...
        Let $\mathit{old}_i$ be the workload-\gls{metric} (already normalized after $G$ has been initialized) already stored in the graph and $\mathit{new}_i$ the new workload-\gls{metric} (normalized by new mean, see equation~\eqref{eq:workloads}).
        \begin{equation}
        \label{eq:euler}
            \mathit{old}_{i+1} = \mathit{old}_i + (\mathit{new}_i - \mathit{old}_i) \cdot \mathit{correction}
        \end{equation}
        In each \gls{balancing}-iteration, after the workload-\gls{metric} is updated (as in equation~\eqref{eq:euler}), every value of the workload-\gls{metric} smaller than a predefined minimum value $\epsilon$ (\eg\ \num{0.1}) is set to $\epsilon$.
        ...
        Therefore, another approach, replacing the \gls{explicit_euler} from equation~\eqref{eq:euler}, is presented in the following, that does spread the workload well and within only two iterations.
        \begin{equation}
        \label{eq:averaging}
            \mathit{old}_{i+1} = \frac{i \cdot \mathit{old}_i + \mathit{new}_i}{i+1}
        \end{equation}
        This result is treated as before, so equations~\eqref{eq:metric_cleanup} are applied as well.
        From equations~\eqref{eq:averaging}, both $\mathit{old}_1=\mathit{new}_0$ and $\mathit{old}_2=\frac{\mathit{old}_1 + \mathit{new}_1}{2}$ holds before normalization.
        ...
        \todo{%
            ...
            But that's the point, the workload-metric should balance.
        }

\section{Executing queries on the balanced graph for evaluation}
