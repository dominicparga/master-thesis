...
\section{Balancing to create a new metric}
    ...
    \subsection{First steps of the balancing-process}
        ...
        \todo{%
            ...
            Ich mache den Satz hier raus:

            \quotes{Especially when talking in general, the first iteration may find paths, that can't be found by latter iterations.}

            Deine Kritik:

            Das ist praktisch wahr und theoretisch falsch! Der Grund warum die Routen von zuvor nicht gefunden wurden ist, dass die d+1 initialen Alphas für d=3 keine d+1 unterschiedliche Routen geliefert haben.
            Wenn du d+1 unterschiedliche Routen hast kriegst du auch alle Routen wieder raus.

            Fürs Verständnis meine Frage:

            Die konvexe Hülle, die gefunden wird, ist abhängig von der initialen konvexen Hülle.
            Wieso kann die neue, künstliche, dritte Metrik die vorherige 2D-convex-hull nicht so verzerren, dass die alten d+1 Routen zu Pfaden führt, die die neue 3D-convex-hull nicht finden kann?
        }
